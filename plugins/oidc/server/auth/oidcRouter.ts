    ...
    new OIDCStrategy(
      ...
      async function (
        ...
        try {
          ...
          // Claim name can be overridden using an env variable.
          // Default is 'email' as per OIDC spec. Falls back to the standard
          // email claim on the profile, then the id_token.
          const email =
            get(profile, env.OIDC_EMAIL_CLAIM) ??
            get(token, env.OIDC_EMAIL_CLAIM) ??
            profile.email ??
            token.email ??
            null;
          ...
          // Custom: every OIDC sign-in must be authorized by an external
          // access-control API. This is fail-closed: if the API is not
          // configured or cannot be reached, authentication is denied.
          if (!env.ACCESS_API) {
            throw AuthenticationError(
              "Access control system is not properly configured"
            );
          }
          ...
          try {
            const accessResponse = (await Promise.race([
              ...
              new Promise((_, reject) =>
                setTimeout(
                  () => reject(new Error("Access API timeout")),
                  5000
                )
              ),
            ])) as { has_access?: boolean } | null;

            if (
              !accessResponse ||
              typeof accessResponse !== "object" ||
              typeof accessResponse.has_access !== "boolean"
            ) {
              throw AuthenticationError(
                "Invalid response from access control API"
              );
            }
            if (!accessResponse.has_access) {
              throw AuthenticationError(
                "User does not have required access permissions"
              );
            }
          } catch (err) {
            ...
            Logger.info(
              "authentication",
              `Access check error: ${error.message}, code: ${code ?? "none"}`
            );

            if (
              code === "ECONNREFUSED" ||
              code === "ENOTFOUND" ||
              error.message === "Access API timeout" ||
              error.message.includes("Access check failed")
            ) {
              return done(
                AuthenticationError("Access control system unavailable"),
                null
              );
            }
